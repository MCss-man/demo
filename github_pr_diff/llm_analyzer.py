...
class LLMAnalyzer:
    ...
    def analyze_risks(self, diff_text: str, min_confidence: str = "low", full_contents: Optional[Dict[str, str]] = None) -> dict:
        """分析 diff 中的潜在风险"""
        if not self.available:
            return {"error": "LLM 客户端不可用"}

        try:
            result = analyze_risks(diff_text, min_confidence=min_confidence, api_key=self.api_key, full_contents=full_contents)
            return result
        except Exception as e:
    ...
    def analyze_full_review(self, diff_text: str, min_confidence: str = "low", full_contents: Optional[Dict[str, str]] = None) -> dict:
        ...
        risks_result = self.analyze_risks(diff_text, min_confidence=min_confidence, full_contents=full_contents)
